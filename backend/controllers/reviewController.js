...
exports.createReview = async (req, res) => {
  try {
    ...
    // 3. Calculate metrics
    const averageRating = (Number(quality) + Number(timeliness) + Number(communication)) / 3;
    ...
    // 5. Update Vendor Trust Score & Stats
    if (vendorId) {
      ...
      // Optional: scale appropriately
      const calculatedTrustScore = newGlobalAverage * Math.log(totalReviews + 1) * 10; 
      
      await User.findByIdAndUpdate(vendorId, {
        $set: {
          "profile.averageRating": newGlobalAverage,
          "profile.totalReviews": totalReviews,
          "profile.trustScore": Math.min(100, calculatedTrustScore)
        }
      });
    }
    ...
    if (productId) {
      ...
      const productTrustScore = newProductAverage * Math.log(productTotalReviews + 1) * 10;

      await FarmerListing.findByIdAndUpdate(productId, {
        $set: {
          averageRating: newProductAverage,
          totalReviews: productTotalReviews,
          trustScore: Math.min(100, productTrustScore)
        }
      });
    }
