...
const FarmerProductListings = () => {
  ...
  const fetchPublicRequests = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/buy-requests/public', {
        headers: { 'x-auth-token': token }
      });
      if (response && response.data && response.data.success) {
      ...
      console.error("Public requests fetch failed:", error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || error.response?.data?.error || "Failed to register product.";
      alert(msg);
    } finally {
      setLoading(false);
    }
        ...
        <div className="filters-section bg-white p-[1.5rem] rounded-[12px] shadow-[0_4px_6px_rgba(0,0,0,0.05)] mb-8">
          <div className="flex flex-col w-full gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div className="filter-group">
                <label><ImageIcon size={14} style={{ display: 'inline', marginRight: 4 }} /> Product Image</label>
                <input
                  type="file"
                  name="productImage"
                  accept="image/*"
                ...
                {formData.productImage && (
                  <div style={{ marginTop: '10px', width: '100px', height: '100px', borderRadius: '8px', overflow: 'hidden', border: '1px solid #e5e7eb' }}>
                    <img 
                  ...
                  <div className="order-id-date flex items-center gap-4">
                    {item.media && item.media[0] && (
                      <div style={{ 
                        ...
                        <img
                          src={item.media[0].url.startsWith('http') ? item.media[0].url : `http://localhost:5000${item.media[0].url}`}
                          alt="thumbnail"
                          style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                        />
                      </div>
